...
def r_crit(m):
    ...
    for i in range(len(table_values.keys())):
        if m == list(table_values.keys())[i]:
            return list(table_values.values())[i]                  # повертае табличе значення функції Романовського якщо m є в таблиці.
        if m < list(table_values.keys())[i]:
            right_dist = abs(list(table_values.keys())[i] - m)
            left_dist = abs(list(table_values.keys())[i - 1] - m)
            if left_dist < right_dist:
                return list(table_values.keys())[i - 1]            # якщо m ближче к лівому значенню повернуть ліве значення.
            else:
                return list(table_values.keys())[i]                # в інших випадках повернуть праве значення.
...
def main():
    ...
    r_kr = r_crit(m)  # створюємо змінну r_kr та присвоюєм їй значення функції r_crit котра приймае аргумент m та поверт найближче табличне значення.
