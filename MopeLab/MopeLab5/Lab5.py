...
KohrenTable = {3: 0.6841, 4: 0.6287, 5: 0.5892, 6: 0.5598, 7: 0.5365, 8: 0.5175, 9: 0.5017, 10: 0.4884,
               range(11, 17): 0.4366, range(17, 37): 0.3720, range(37, 145): 0.3093}

StudentTable = {8: 2.306, 9: 2.262, 10: 2.228, 11: 2.201, 12: 2.179, 13: 2.160, 14: 2.145, 15: 2.131, 16: 2.120,
...
def append_to_list_x(x: list, variant: bool):
    if not variant:
...
def main(m, n):
    ...
    if n == 15:
        ...
        append_to_list_x(norm_x, variant=True)
        append_to_list_x(x, variant=True)
    ...
    if n == 8:
        ...
        append_to_list_x(norm_x, variant=False)
        append_to_list_x(x, variant=False)
    ...
    if n == 15:
        t = PrettyTable(['N', 'norm_x_0', 'norm_x_1', 'norm_x_2', 'norm_x_3', 'norm_x_1_x_2', 'norm_x_1_x_3',
                         'norm_x_2_x_3', 'norm_x_1_x_2_x_3', 'norm_x_1^2', 'norm_x_2^2', 'norm_x_3^2', 'x_0',
                         'x_1', 'x_2', 'x_3', 'x_1_x_2', 'x_1_x_3', 'x_2_x_3', 'x_1_x_2_x_3', 'x_1^2', 'x_2^2',
    ...
    if n == 15:
        print(
            ...
            f"{b_i[5]:.5f} * x1 * x3 + {b_i[6]:.5f} * x2 * x3 + {b_i[7]:.5f} * x1 * x2 * x3 + {b_i[8]:.5f} * x1^2 + "
            f"{b_i[9]:.5f} * x2^2 + {b_i[10]:.5f} * x3^2")
    if n == 8:
        print(
            f"\nThe naturalized regression equation: "
            f"y = {b_i[0]:.5f} + {b_i[1]:.5f} * x1 + {b_i[2]:.5f} * x2 + "
    ...
    for i in range(len(t)):
        if get_value(StudentTable, f_3) > t[i]:
            beta_i[i] = 0
            d -= 1
    if n == d:
        n = 8 if n == 4 else 15
    ...
    if f_p > get_value(FisherTable, f_3)[f_4]:
        n = 8 if n == 4 else 15
        print(
            f"fp = {f_p} > ft = {get_value(FisherTable, f_3)[f_4]}.\n"
...
# If you start program with 4 then you will unlikely reach 15
main(m=3, n=15)
