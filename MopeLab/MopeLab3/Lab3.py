...
def main(m, n):
    ...
    print("\nСреднє з характеристик відповіді:")
    ...
    if (gP < gT):
        ...
        print(f"Дисперсія не являєтся однорідною Gp = {gP:.5} < Gt = {gT}")
        m = m + 1
        main(m + 1, n, q)
        return
    ...
    print("Нормалізоване значення : ", ckeck_list)
    ...
    if (fP > fT[f3][f_4]):
        print(f"fp = {fP} > ft = {fT[f3][f_4]}.\nМатематична модель не адекватна экспериментальним данним\n")
    else:
        print(f"fP = {fP} < fT = {fT[f3][f_4]}.\nМатематична модель  адекватна экспериментальним данним\n")
